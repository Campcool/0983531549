...
$albums = @(
  ...
  # 2026-09-11：油漆清潔來源第 7／8 張是橫躺的，需先順時針轉正再加浮水印。
  # 2026-09-12：前台 paint-cleaning-09.jpg 是業主以 ChatGPT 附件補的完工照，
...
foreach ($album in $albums) {
  ...
  $files | ForEach-Object {
      ...
      try {
        if ([object[]]$album.RotateClockwiseIndexes -contains $index) {
